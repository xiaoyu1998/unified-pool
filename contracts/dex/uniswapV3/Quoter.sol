...
contract Quoter is IQuoter, IUniswapV3SwapCallback, PeripheryImmutableState {
    ...
    function quoteExactInputSingle(
    ...
    ) public override returns (uint256 amountOut, uint256 startSqrtPrice) {
        QuoteLocalVars memory vars;
        vars.zeroForOne = tokenIn < tokenOut;

        vars.pool = getPool(tokenIn, tokenOut, fee);
        (vars.startSqrtPriceX96,,,,,,) = vars.pool.slot0();
        ...
        {} catch (bytes memory reason) {
            return (parseRevertReason(reason), vars.startSqrtPriceX96);
        }
    }
    ...
    function quoteExactOutputSingle(
        ...
        uint256 amountOut,
        uint160 sqrtPriceLimitX96
    ) public override returns (uint256 amountIn, uint256 startSqrtPrice) {
        QuoteLocalVars memory vars;
        vars.zeroForOne = tokenIn < tokenOut;
